# BFR-00 / WP-2 — Scope-switch flow Gherkin
#
...
#   - citrate_v0.01.1/gui/citrate_boeing_kit/ui/primitives/tenant_scope_chip.slint
#   - citrate_v0.01.1/gui/citrate_boeing_kit/ui/composables/modal.slint (confirm)
#
...
Feature: Scope-switch flow — picker, classification confirm, signed apply
  ...
  Background:
    Given the operator's active scope is "Boeing > BCA > Everett > 777X"
  ...
  Scenario: Switching within the same subtree at same clearance applies directly
    Given the operator's scope is "Boeing > BCA > Everett > 777X" (Proprietary)
    When the operator opens the picker
    And selects target "Boeing > BCA > Everett > 787" (Proprietary, same subtree)
  ...
  Scenario: Switching to a lower-clearance scope requires confirmation
    Given the operator's scope is "Boeing > BCA > Everett > 777X" (Proprietary)
    When the operator selects target "Boeing > Public > Marketing" (Public)
  ...
  Scenario: Switching to a different ITAR subtree requires confirmation
    Given the operator's scope is "Boeing > BDS > St.Louis > KC-46" (ITAR subtree A)
    When the operator selects target "Boeing > BDS > Mesa > AH-64" (ITAR subtree B)
