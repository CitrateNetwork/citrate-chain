...
/// @title CrossOrgEnvelope — Multi-org envelope flow for DPF-14.
/// @notice Per planset 09_INTER_ORG_TRANSFER.md § Architecture +
///         InterOrgEnvelopeChain.tla. Generalizes the single-org
///         envelope to span multiple tenant roots (DefensePrime → Tier-1 →
...
///        - threshold > 0 per org
///
/// Source: .agentile/sprints/active/2026-05-11-dpf-14-inter-org-transfer/SPRINT.md D-1
contract CrossOrgEnvelope {
    ...
    /// @notice Raised when a caller signs for an org for which it is not
    ///         a registered recorder. Closes CHAIN-B-C026 (a).
    error NotOrgRecorder(bytes32 org_root, address caller);
    ...
    /// @notice Per-org recorder allowlist. `sign(...)` for an org requires
    ///         the caller be a recorder registered FOR THAT org, so one
    ///         global recorder key cannot satisfy every organization's
    ///         threshold. Closes CHAIN-B-C026 (a).
    mapping(bytes32 => mapping(address => bool)) public is_org_recorder;
    ...
    event OrgRecorderSet(bytes32 indexed org_root, address indexed recorder, bool authorized);
    ...
    /// @notice Authorize (or revoke) a recorder FOR a specific org root.
    ///         Governance-only. A caller may only `sign` on behalf of an
    ///         org it is registered for. Closes CHAIN-B-C026 (a).
    function setOrgRecorder(bytes32 org_root, address recorder, bool authorized) external {
        if (msg.sender != governance) revert NotGovernance(msg.sender);
        is_org_recorder[org_root][recorder] = authorized;
        emit OrgRecorderSet(org_root, recorder, authorized);
    }
    ...
    function sign(bytes32 envelope_id, bytes32 org_root, bytes32 signer) external {
        // CHAIN-B-C026 (a): the caller must be a recorder registered for
        // THIS org, so one key cannot forge every organization's threshold.
    ...
    /// @notice Mark Signed → Delivered. Recorder-gated (CHAIN-B-C026 (b)):
    ///         previously permissionless, which let anyone drive a freshly
    ///         Signed envelope straight to terminal-positive `Closed`,
    ///         erasing the rejection window.
    ...
    /// @notice Mark Delivered → Accepted. Recorder-gated (CHAIN-B-C026 (b)).
