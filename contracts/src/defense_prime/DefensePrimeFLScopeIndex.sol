...
///         the DPF-07 Federated Learning panel can scope-filter
...
///         "which pool ids belong to a DefensePrime scope?".
///
...
///     tag (no permissionless tagging — DefensePrime operations team
///     decides which pools are part of DefensePrime's FL program).
///
/// @dev DPF-07 deliverable. Companion to RM-FL track's
///      `LearningPool.sol` + `ContributionAccounting.sol`. The DefensePrime
///      panel adapter (`citrate-defense_prime-fl`) is responsible for the
///      `poolId → pool detail` and `poolId → reward split` joins.
contract DefensePrimeFLScopeIndex {
    // ── State ───────────────────────────────────────────────────────

    /// @notice Pool id → DefensePrime scope (zero = untagged).
    mapping(uint256 pool_id => bytes32) public poolScope;

    /// @notice DefensePrime scope → list of pool ids.
    mapping(bytes32 scope => uint256[]) private _pools_by_scope;

    /// @notice Authorized taggers (DefensePrime ops team).
    ...
    /// @notice Tag a pool with a DefensePrime scope. One-shot per pool.
