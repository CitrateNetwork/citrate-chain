...
/// @title RoleGrantTenantIndex — Scope→user index sidecar (BFR-11)
/// @notice Maintains `mapping(bytes32 scope => bytes32[] user_ids)`
///         observed at grant time. Pairs with the existing
///         `RoleEscalation` (BFR-02 at `0x3130B9...`) which doesn't
...
///      `boeing-gov-roles(scope) → RoleEscalation.byTenant(scope) + ...`.
...
///
/// @dev Same pattern as BFR-09's CrossOrgIndex (scope→envelopes).
///      Tests, governance gating, and storage layout intentionally
///      mirror it for consistency.
///
/// Source: .agentile/sprints/active/2026-05-11-bfr-11-governance-panel/SPRINT.md D-1
