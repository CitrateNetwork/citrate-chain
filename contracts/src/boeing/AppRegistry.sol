...
/// @notice Minimal interface against `MultiSigEnvelope` for the
///         threshold check. AppRegistry only needs the boolean view;
///         the envelope contract is otherwise opaque to AppRegistry.
interface IEnvelopeOracle {
    function isSignedThresholdMet(bytes32 envelope_id) external view returns (bool);
}

/// @title AppRegistry — Boeing app + contract registry (BFR-09)
...
///     envelope has `isSignedThresholdMet == true`.
...
contract AppRegistry {
    ...
    /// @notice Address of the MultiSigEnvelope contract (BFR-02).
    ///         Used solely for the threshold-met check in `deploy`.
    IEnvelopeOracle public envelope_oracle;
    ...
    /// @notice All contracts ever registered, for the `boeing-contracts-list`
    ///         IPC. Append-only.
    ...
    constructor(address initialGovernance, address envelopeOracle) {
        ...
        envelope_oracle = IEnvelopeOracle(envelopeOracle);
    }
    ...
    function deploy(bytes32 app_id) external {
        ...
        if (!envelope_oracle.isSignedThresholdMet(a.deploy_envelope)) {
