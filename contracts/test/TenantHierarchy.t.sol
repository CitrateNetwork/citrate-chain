...
/// @title TenantHierarchy.t — DPF-02-WP2 Forge tests
...
contract TenantHierarchyTest is Test {
    ...
    bytes32 constant ROOT = keccak256("DefensePrime");
    bytes32 constant CommercialUnit = keccak256("DefensePrime.CommercialUnit");
    bytes32 constant EVERETT = keccak256("DefensePrime.CommercialUnit.Everett");
    bytes32 constant LINE = keccak256("DefensePrime.CommercialUnit.Everett.787-Line");
    bytes32 constant BDS = keccak256("DefensePrime.BDS");
    ...
    function setUp() public {
        ...
        admins[0] = admin1;
        admins[1] = admin2;
        th.initRoot(ROOT, "DefensePrime", bytes32(uint256(1)), admins, 2, 3);
    }
    ...
    function test_initRoot_revertsOnDoubleInit() public {
        ...
        th.initRoot(CommercialUnit, "x", bytes32(0), admins, 1, 0);
    }
    ...
    function test_createNode_levelOne() public {
        _createBcaUnderRoot();
        assertTrue(th.exists(CommercialUnit));
        TenantHierarchy.TenantNode memory n = th.getNode(CommercialUnit);
    ...
    function test_createNode_levelTwo() public {
        ...
        assertEq(n.level, 2);
        assertEq(n.parent, CommercialUnit);
    }
    ...
    function test_createNode_emitsNodeCreated() public {
        ...
        emit TenantHierarchy.NodeCreated(CommercialUnit, ROOT, 1);
        th.createNode(ROOT, CommercialUnit, "CommercialUnit", 1, bytes32(uint256(2)), admins, 1, 3);
    }
    ...
    function test_createNode_revertsWhenParentMissing() public {
        ...
        th.createNode(ghostParent, CommercialUnit, "x", 1, bytes32(0), admins, 1, 0);
    }
    ...
    function test_createNode_revertsOnDuplicateSelf() public {
        ...
        vm.prank(admin1);
        vm.expectRevert(
            abi.encodeWithSelector(TenantHierarchy.NodeAlreadyExists.selector, CommercialUnit)
        );
        th.createNode(ROOT, CommercialUnit, "x", 1, bytes32(0), admins, 1, 0);
    }
    ...
    function test_createNode_revertsOnLevelMismatch() public {
        ...
        th.createNode(ROOT, CommercialUnit, "x", 2, bytes32(0), admins, 1, 0);
    }
    ...
    function test_createNode_revertsOnInvalidLevelZero() public {
        ...
        th.createNode(ROOT, CommercialUnit, "x", 0, bytes32(0), admins, 1, 0);
    }
    ...
    function test_createNode_revertsWhenChildClearanceExceedsParent() public {
        // Make CommercialUnit's max = 1 (Proprietary), then try to create child with max=2.
        address[] memory admins = new address[](1);
        admins[0] = admin1;
        vm.prank(admin1);
        th.createNode(ROOT, CommercialUnit, "CommercialUnit", 1, bytes32(uint256(2)), admins, 1, 1);
        ...
        th.createNode(CommercialUnit, EVERETT, "E", 2, bytes32(0), admins, 1, 2);
    }
    ...
    function test_setClassificationMax_revertsWhenExceedsParent() public {
        ...
        // so first lower CommercialUnit to 1 before lowering ROOT to 1 (otherwise the
        // ChildExceedsClassification guard fires — which its own test covers).
        vm.prank(admin1);
        th.setClassificationMax(CommercialUnit, 1);
        // Lower root's max to 1.
        vm.prank(admin1);
        th.setClassificationMax(ROOT, 1);
        // Now raising CommercialUnit back to 3 exceeds the parent's (root's) max of 1.
        vm.prank(admin1);
        vm.expectRevert(
            abi.encodeWithSelector(
        ...
        th.setClassificationMax(CommercialUnit, 3);
    }
    ...
    function test_setClassificationMax_emitsNodeUpdated() public {
        ...
        emit TenantHierarchy.NodeUpdated(
            CommercialUnit, keccak256("classification_max"), abi.encode(uint8(1))
        );
        th.setClassificationMax(CommercialUnit, 1);
    }
    ...
    function test_removeNode_succeedsWhenChildless() public {
        _createBcaUnderRoot();
        vm.prank(admin1);
        th.removeNode(CommercialUnit);
        assertFalse(th.exists(CommercialUnit));
    }
    ...
    function test_removeNode_revertsWhenHasChildren() public {
        ...
        vm.prank(admin1);
        vm.expectRevert(
            abi.encodeWithSelector(TenantHierarchy.HasChildren.selector, CommercialUnit)
        );
        th.removeNode(CommercialUnit);
    }
    ...
    function test_removeNode_emitsNodeRemoved() public {
        ...
        vm.prank(admin1);
        vm.expectEmit(true, true, false, true);
        emit TenantHierarchy.NodeRemoved(CommercialUnit, admin1);
        th.removeNode(CommercialUnit);
    }
    ...
    function test_getPath_levelOne() public {
        _createBcaUnderRoot();
        bytes32[] memory p = th.getPath(CommercialUnit);
        assertEq(p.length, 2);
        assertEq(p[0], ROOT);
        assertEq(p[1], CommercialUnit);
    }
    ...
    function test_deriveSubSecretSalts_returnsRootToLeafChain() public {
        ...
        // ROOT salt was 1 in setUp, CommercialUnit was 2, EVERETT was 3.
        ...
        public
    {
        ...
        if (child_max > parent_max) {
            vm.expectRevert();
            th.createNode(ROOT, CommercialUnit, "x", 1, bytes32(0), admins, 1, child_max);
        } else {
            th.createNode(ROOT, CommercialUnit, "x", 1, bytes32(0), admins, 1, child_max);
            assertLe(th.getNode(CommercialUnit).classification_max, parent_max);
        }
    }
    ...
    function testFuzz_LevelBounded(uint8 level) public {
        ...
        if (level == 0 || level > 3) {
            ...
            th.createNode(ROOT, CommercialUnit, "x", level, bytes32(0), admins, 1, 0);
        } else if (level != 1) {
            vm.expectRevert(
            ...
            th.createNode(ROOT, CommercialUnit, "x", level, bytes32(0), admins, 1, 0);
            assertEq(th.getNode(CommercialUnit).level, 1);
        }
    }
    ...
    function _createEverettUnderBca() internal {
        ...
        admins[0] = admin1;
        vm.prank(admin1);
        th.createNode(CommercialUnit, EVERETT, "Everett", 2, bytes32(uint256(3)), admins, 1, 3);
    }
