...
contract RmQ_C026 is Test {
    ...
    address internal boeingRec = address(0xB0E);
    ...
    function setUp() public {
        ...
        // A recorder authorized ONLY for the Boeing org.
        vm.prank(governance);
        env.setOrgRecorder(BOEING, boeingRec, true);
        _draft();
    }
    ...
    /// RED (a): the Boeing recorder key tries to also satisfy Tier-1.
    ...
    function test_C026_recorder_cannot_forge_other_orgs_signature() public {
        vm.prank(boeingRec);
        env.sign(ENV_1, BOEING, SIG_B1); // legitimate for its own org

        vm.prank(boeingRec);
        vm.expectRevert(
            abi.encodeWithSelector(CrossOrgEnvelope.NotOrgRecorder.selector, TIER1, boeingRec)
        );
        env.sign(ENV_1, TIER1, SIG_T1);

        // GREEN: Tier-1 threshold NOT met off the Boeing key alone.
    ...
    function test_C026_anon_cannot_advance_delivery() public {
        vm.prank(governance);
        env.setOrgRecorder(TIER1, boeingRec, true); // give a real per-org path
        vm.startPrank(boeingRec);
