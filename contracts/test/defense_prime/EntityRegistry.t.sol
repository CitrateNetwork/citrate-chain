...
contract EntityRegistryTest is Test {
    ...
    function test_register_only_recorder() public {
        ...
        reg.registerType(SCOPE_UNIT, "Part", "Aircraft part", CID_1);
    }
    ...
    function test_register_rejects_duplicate() public {
        ...
        bytes32 id = reg.registerType(SCOPE_UNIT, "Part", "x", CID_1);
        vm.expectRevert(abi.encodeWithSelector(EntityRegistry.AlreadyRegistered.selector, id));
        reg.registerType(SCOPE_UNIT, "Part", "y", CID_2);
        vm.stopPrank();
    }

    function test_register_emits_event() public {
        bytes32 expected = keccak256(abi.encode(SCOPE_UNIT, "Part"));
        vm.expectEmit(true, true, false, true);
        emit TypeRegistered(expected, SCOPE_UNIT, CID_1, "Part");
        vm.prank(recorder);
        reg.registerType(SCOPE_UNIT, "Part", "x", CID_1);
    }
    ...
    function test_typesByScope_appends_once_per_register() public {
        vm.startPrank(recorder);
        reg.registerType(SCOPE_UNIT, "Part", "x", CID_1);
        reg.registerType(SCOPE_UNIT, "Tail", "y", CID_1);
        reg.registerType(SCOPE_BDS, "Supplier", "z", CID_1);
        vm.stopPrank();
        assertEq(reg.types(SCOPE_UNIT).length, 2);
    ...
    function test_typesByScope_does_not_grow_on_bump() public {
        ...
        // doesn't push.
        assertEq(reg.types(SCOPE_UNIT).length, 1);
    }
    ...
    function test_register_two_types_different_scopes_same_name() public {
        ...
        vm.startPrank(recorder);
        bytes32 id1 = reg.registerType(SCOPE_UNIT, "Part", "x", CID_1);
