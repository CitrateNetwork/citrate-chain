...
contract TinaWorkpaperRegistryTest is Test {
    ...
    function test_draft_rejects_duplicate() public {
        ...
        reg.draftWorkpaper(WP_1, PO_1, ROOT, CID, SCOPE_UNIT, 3, block.number + 2000);
        vm.stopPrank();
    }
    ...
    function test_addSignature_rejects_when_not_pending() public {
        vm.startPrank(recorder);
        reg.draftWorkpaper(WP_1, PO_1, ROOT, CID, SCOPE_UNIT, 1, block.number + 1000);
        reg.addSignature(WP_1, SIG_A);
        vm.stopPrank();
    ...
    function test_signersList_records_order() public {
        vm.startPrank(recorder);
        reg.draftWorkpaper(WP_1, PO_1, ROOT, CID, SCOPE_UNIT, 3, block.number + 1000);
        reg.addSignature(WP_1, SIG_A);
        reg.addSignature(WP_1, SIG_B);
    ...
    function test_expire_after_deadline_transitions_to_expired() public {
        ...
        reg.draftWorkpaper(WP_1, PO_1, ROOT, CID, SCOPE_UNIT, 2, deadline);
        vm.roll(deadline + 1);
        reg.expireWorkpaper(WP_1);
    ...
    function test_expire_already_signed_reverts() public {
        vm.startPrank(recorder);
        reg.draftWorkpaper(WP_1, PO_1, ROOT, CID, SCOPE_UNIT, 1, block.number + 10);
        reg.addSignature(WP_1, SIG_A);
        vm.stopPrank();
        reg.signWorkpaper(WP_1);
    ...
    function test_byScope_appends() public {
        vm.startPrank(recorder);
        reg.draftWorkpaper(WP_1, PO_1, ROOT, CID, SCOPE_UNIT, 2, block.number + 1000);
        reg.draftWorkpaper(WP_2, PO_1, ROOT, CID, SCOPE_UNIT, 2, block.number + 1000);
        vm.stopPrank();
