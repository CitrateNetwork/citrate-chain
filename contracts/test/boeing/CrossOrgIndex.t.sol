...
contract CrossOrgIndexTest is Test {
    ...
    function test_record_emits_event_and_grows_byScope() public {
        ...
        emit CrossOrgIndex.Indexed(SCOPE_BCA, ENV_1);
        vm.prank(recorder);
        idx.record(SCOPE_BCA, ENV_1);

        bytes32[] memory list = idx.byCrossOrg(SCOPE_BCA);
        assertEq(list.length, 1);
        assertEq(list[0], ENV_1);
        assertEq(idx.envelopeScope(ENV_1), SCOPE_BCA);
        assertTrue(idx.is_indexed(ENV_1));
        assertEq(idx.countByScope(SCOPE_BCA), 1);
    }
    ...
    function test_multiple_scopes_are_isolated() public {
        ...
        assertEq(idx.byCrossOrg(SCOPE_BCA).length, 1);
        assertEq(idx.byCrossOrg(SCOPE_BDS).length, 1);
        assertEq(idx.byCrossOrg(SCOPE_BCA)[0], ENV_1);
    ...
    function test_record_appends_in_order() public {
        ...
        vm.prank(recorder);
        idx.record(SCOPE_BCA, e1);
        vm.prank(recorder);
        idx.record(SCOPE_BCA, e2);
        vm.prank(recorder);
        idx.record(SCOPE_BCA, e3);
        bytes32[] memory list = idx.byCrossOrg(SCOPE_BCA);
