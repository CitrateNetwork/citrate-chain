...
contract RoleGrantTenantIndexTest is Test {
    ...
    function test_record_emits_event_and_grows_byTenant() public {
        ...
        emit RoleGrantTenantIndex.Indexed(SCOPE_BCA, USER_1);
        vm.prank(recorder);
        idx.record(SCOPE_BCA, USER_1);

        bytes32[] memory list = idx.byTenant(SCOPE_BCA);
        assertEq(list.length, 1);
        assertEq(list[0], USER_1);
        assertEq(idx.countByScope(SCOPE_BCA), 1);
        assertTrue(idx.isIndexed(SCOPE_BCA, USER_1));
    }
    ...
    function test_empty_byTenant_returns_empty() public view {
        assertEq(idx.byTenant(SCOPE_BCA).length, 0);
        assertEq(idx.countByScope(SCOPE_BCA), 0);
        assertFalse(idx.isIndexed(SCOPE_BCA, USER_1));
    }

    function test_multiple_users_under_same_scope() public {
        vm.startPrank(recorder);
        idx.record(SCOPE_BCA, USER_1);
        idx.record(SCOPE_BCA, USER_2);
        vm.stopPrank();
        bytes32[] memory list = idx.byTenant(SCOPE_BCA);
