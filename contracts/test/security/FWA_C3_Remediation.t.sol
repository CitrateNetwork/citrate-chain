...
contract FWA_C3_Remediation is Test {
    ...
    function test_C3_02_non_deployer_cannot_initRoot() public {
        ...
        th.initRoot(keccak256("DefensePrime"), "DefensePrime", bytes32(uint256(1)), admins, 1, 3);

        // The deployer can still init exactly once.
        address[] memory good = new address[](1);
        good[0] = address(0xA1);
        th.initRoot(keccak256("DefensePrime"), "DefensePrime", bytes32(uint256(1)), good, 1, 3);
        assertEq(th.root(), keccak256("DefensePrime"));
    }
