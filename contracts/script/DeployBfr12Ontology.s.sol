...
import "../src/boeing/EntityRegistry.sol";

/// @title DeployBfr12Ontology — Stage 10 broadcast
/// @notice Deploys the BFR-12 NEW contract (EntityRegistry) to chain
///         40204. Deployer self-authorizes as recorder so the E2E
///         suite can write without a separate cast send.
contract DeployBfr12Ontology is ScriptEnv {
    function run() external returns (address entityRegistry) {
        address deployer = deployerAddress();
        address governance = envAddressOr("GOVERNANCE", deployer);

        console.log("=== BFR-12 Ontology deployment (Stage 10) ===");
