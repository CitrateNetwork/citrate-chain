...
/// @title DeployBfr05Provenance — Foundry deployment script for BFR-05 Provenance
...
///      `src/boeing/PartProvenanceRegistry.sol`). The same `_governance()`
///      env-or-msg.sender pattern as `DeployBfr02Rbac.s.sol` is used so
...
///      after BFR-02's ContradictionLedger is also live, the operator
///      should call:
...
///   forge script script/DeployBfr05Provenance.s.sol \
...
///   ROOT_GOVERNANCE=0x... forge script script/DeployBfr05Provenance.s.sol \
///       --rpc-url https://rpc.citrate.ai \
...
contract DeployBfr05Provenance is Script {
    ...
    function run() external returns (DeployedAddresses memory addrs) {
        ...
        // PartProvenanceRegistry — single contract for BFR-05.
        ...
        console2.log("=== BFR-05 Provenance deployment complete ===");
        ...
        console2.log("Operator follow-up: setContradictionLedger(<ledger>) once BFR-02 Stage 1 is live");
    }
}
