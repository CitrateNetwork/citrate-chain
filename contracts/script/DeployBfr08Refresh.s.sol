...
/// @title DeployBfr08Refresh — Refresh the 3 drifted BFR-08 read-path contracts
...
///
///      Per the established BFR pattern, `--broadcast` is HUMAN-IN-LOOP
///      (Saul holds the deployer key in .env.testnet).
///
/// Usage:
///   # Dry-run:
///   forge script script/DeployBfr08Refresh.s.sol \
...
contract DeployBfr08Refresh is ScriptEnv {
    ...
    function run() external returns (DeployedAddresses memory addrs) {
        ...
        console.log("=== BFR-08 read-path contract refresh ===");
