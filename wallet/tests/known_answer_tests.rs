...
// ============================================================
// 5. Keystore Argon2 params meet OWASP minimums
// ============================================================

#[test]
fn test_keystore_argon2_params_minimum() {
    // The keystore uses Argon2::default() which is argon2 0.5.x.
    ...
    assert!(
        ...
        "Argon2 memory cost must be >= 19456 KiB (OWASP minimum), got {} KiB",
        params.m_cost()
    );

    assert!(
        params.t_cost() >= 2,
        "Argon2 iterations must be >= 2 (OWASP minimum), got {}",
        params.t_cost()
    );
}
