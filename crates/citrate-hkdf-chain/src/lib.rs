//! DPF-14 — HKDF sub-secret chain.
//!
//! Implements `HKDFSubSecretDerivation.tla` (DPF-00):
//!
//! ```text
...
//! - DefensePrime's CommercialUnit-Everett-777X team holds a sub-secret derived through
//!   3 HKDF steps from the DefensePrime root.
...
/// - `label` — UTF-8 child label (e.g., `"CommercialUnit"`, `"Everett"`, `"777X"`).
///
/// # Returns
...
mod tests {
    ...
    fn derive_different_labels_yield_different_children() {
        let parent = fill(0xAA);
        let salt = fill(0x11);
        let bca = derive(&parent, &salt, b"CommercialUnit").expect("ok");
    ...
    fn derive_chain_3_levels_matches_manual_composition() {
        ...
        let manual_a = derive(&root, &salt_a, b"CommercialUnit").expect("ok");
    ...
    fn sibling_derivations_are_independent() {
        // Knowing the CommercialUnit child gives no information about BDS child
    ...
    #[test]
    fn defense_prime_demo_path_matches_planset_example() {
        // Planset § HKDF chain: DefensePrime root → CommercialUnit → Everett → 777X.
        let root = fill(0x00);
        let salts = [fill(0x01), fill(0x02), fill(0x03)];
        let r = derive_chain(
            &root,
            &[
                (&salts[0], b"CommercialUnit"),
                (&salts[1], b"Everett"),
                (&salts[2], b"777X"),
            ],
        )
    ...
    fn derive_returns_zeroizing_key_material() {
        ...
        let parent = fill(0xAA);
        let salt = fill(0x11);
        let child = derive(&parent, &salt, b"CommercialUnit").expect("ok");
        assert_is_zeroizing(&child);
        let chained = derive_chain(&parent, &[(&salt, b"CommercialUnit")]).expect("ok");
