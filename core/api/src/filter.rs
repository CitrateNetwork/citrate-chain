...
impl FilterRegistry {
    ...
    pub fn new_log_filter(
        ...
        self.filters.write().unwrap_or_else(|e| e.into_inner()).insert(id, filter);
        id
    }
    ...
    pub fn update_last_poll_block(&self, id: u64, block: u64) {
        if let Some(filter) = self.filters.write().unwrap_or_else(|e| e.into_inner()).get_mut(&id) {
    ...
    pub fn uninstall_filter(&self, id: u64) -> bool {
        self.filters.write().unwrap_or_else(|e| e.into_inner()).remove(&id).is_some()
    }
