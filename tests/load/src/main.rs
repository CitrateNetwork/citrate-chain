...
async fn main() {
    ...
    while start.elapsed() < duration {
        ...
        if target_tps > 0 && sent % (target_tps * 5) == 0 {
            ...
            let actual_tps = if elapsed > 0.0 { ok as f64 / elapsed } else { 0.0 };
            println!(
    ...
    println!("  Failed               : {}", fail);
    println!("  Success rate         : {:.1}%", ok as f64 / sent as f64 * 100.0);
    println!();
    println!("  Target TPS           : {}", target_tps);
    println!("  Actual TPS           : {:.0}", ok as f64 / elapsed.as_secs_f64());
