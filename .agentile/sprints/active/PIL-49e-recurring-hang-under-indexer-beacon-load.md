...
```bash
ssh root@<RPC_HOST> 'journalctl -u citrate-node --since "2026-06-08" \
...
```
$ ssh root@<RPC_HOST> 'strings /home/citrate/bin/citrate-node \
...
ssh root@<RPC_HOST> 'ss -tlnH "( sport = :8545 )" | awk "{print \$2}"'
# Expect: 1025 or close

# 2. Per-listener accept state — distinguishes accept-loop dead
#    vs workers blocked
ssh root@<RPC_HOST> 'ss -tnp "sport = :8545" | head -20'

# 3. citrate-node stack trace via gcore + gdb on a side process
ssh root@<RPC_HOST> 'gcore -o /tmp/citrate-node $(pgrep -f citrate-node | head -1) &
sleep 6
...
ssh root@<RPC_HOST> 'curl -s http://127.0.0.1:9001/metrics 2>/dev/null \
