...
- **`47.146.64.6`** — an EXTERNAL NAT'd node (not one of the 4 DO droplets) stuck on the old
...
- Neutral: does NOT retroactively fix the current live split — that needs the 47.146.64.6 source
  stopped + boot2 re-wiped (owner is handling 47.146.64.6). This ADR prevents recurrence.
