...
#   - ${ENV_TESTNET:-../.env.testnet} — CITRATE_AA_* pins
...
ENV_TESTNET="${ENV_TESTNET:-${REPO_ROOT}/../.env.testnet}"
...
CEREMONIES=(
  ...
  # Homestead org-scoped SBTs (cit-agent hierarchy): FacilitySBT + NetworkSBT,
  # each deployed behind a UUPS ERC1967Proxy. The PROXY is the canonical,
  # frozen address-book entry (the impl is upgradeable behind it). Because both
  # proxies share the contractName "ERC1967Proxy", the generic name→address
  # merge can't tell them apart — DeployScopedSBTs is special-cased below.
  "DeployScopedSBTs.s.sol"
)
...
for ceremony in "${CEREMONIES[@]}"; do
  ...
  pairs=$(jq -c '
    ...
    | from_entries
  ' "$bc")
  if [[ "$ceremony" == "DeployScopedSBTs.s.sol" ]]; then
    # Both SBTs deploy an impl then an ERC1967Proxy. The proxy is the canonical
    # entry; record it under the SBT's name and the impl under <name>Impl.
    # DeployScopedSBTs.s.sol deploys deterministically in this order:
    #   1) FacilitySBT impl  2) ERC1967Proxy(Facility)
    #   3) NetworkSBT impl   4) ERC1967Proxy(Network)
    # so among the CREATE2 rows the 1st proxy is Facility's and the 2nd is
    # Network's. The impls carry their own contractName.
    pairs=$(jq -c '
      .transactions
      | map(select((.transactionType == "CREATE" or .transactionType == "CREATE2") and .contractAddress != null))
      | . as $txs
      | ($txs | map(select(.contractName == "ERC1967Proxy") | .contractAddress)) as $proxies
      | {
          FacilitySBTImpl: (($txs | map(select(.contractName == "FacilitySBT")) | .[0].contractAddress) // ""),
          NetworkSBTImpl:  (($txs | map(select(.contractName == "NetworkSBT"))  | .[0].contractAddress) // ""),
          FacilitySBT:     (($proxies[0]) // ""),
          NetworkSBT:      (($proxies[1]) // "")
        }
      | with_entries(select(.value != "") | .value |= ascii_downcase)
    ' "$bc")
    jq --argjson new "$pairs" '. + $new' "$contracts_jq" > "${contracts_jq}.tmp" && mv "${contracts_jq}.tmp" "$contracts_jq"
  elif [[ "$ceremony" == "DeployAA.s.sol" || "$ceremony" == "DeployAndPinAA.s.sol" ]]; then
