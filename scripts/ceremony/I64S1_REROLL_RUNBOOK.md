...
| **Owner confirms governance = deployer** | ✅ CONFIRMED 2026-06-29 — `GOVERNANCE` env left unset (defaults to deployer). ⚠️ the addresses in this row are PRE-ROTATION/stale — see the determinism-pins row below for current values. |
| **Determinism env pins (citrate-chain#170 audit, 2026-08-27)** | These env values are ADDRESS-DETERMINING — set them exactly or contracts drift off the book: **`DEPLOYER_ADDRESS=0x4fAB35c8c5033c80b3a0452A873B81e6ED4ED732`** (rotated deployer — NOT the old `0x4250675F…`; the book was regenerated with it), **`GOVERNANCE` unset** (→ deployer), EduStack **`SIGNER_1/2/3` unset** (→ deployer) + **`SALT_USD_RATE=100`**, **`FOLD_VERIFIER` unset** (→ `0x0130`). Verified by simulation: KYC `0xcf41…`, IPFSIncentivesV3 `0xa1a37f…` (asserted in FL), all match the book with these values. **P1 fix:** `TEEAttestationRegistry = 0x4dF26aae…` is the canonical live TEE (was a dead `0xc1c0d858…` pin) → `ComputePoolPipeline` moved to `0x39b2ca87385b7db07a9f333a5951d506a9b3c81d` (book updated). **Governance/DPF suite** (Quorum + `DeployDpf*`) is plain-`new` (non-deterministic) but restored + book-harvested by `post-reroll-quorum-restore.sh` (patches BOTH `dpf-40204.json` and `40204.json`) — no manual patch needed; addresses just aren't stable across rerolls. |
...
# PBA-L1a-003: `commd-fold-verify` is now in citrate-node's DEFAULT feature set (and the binary refuses
...
cd ../citrate-defense_prime-shell      && bash scripts/sync-addresses.sh
